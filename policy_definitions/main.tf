locals {
  policy_definition_files = fileset(var.policy_definitions_path, "**/*.json")
  policy_definitions_list = [for definition_file in local.policy_definition_files : jsondecode(file("${var.policy_definitions_path}${definition_file}"))]
